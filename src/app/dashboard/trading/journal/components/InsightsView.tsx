// src/app/dashboard/trading/journal/components/InsightsView.tsx - PART 1
"use client";
      ...
      <div className={`min-h-screen p-3 sm:p-4 md:p-6 ${isDark ? "bg-slate-900" : "bg-slate-50"}`}>
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            ...
            <button
              onClick={onBack}
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition ${isDark
    ...
    <div className={`min-h-screen p-3 sm:p-4 md:p-6 ${isDark ? "bg-slate-900" : "bg-slate-50"}`}>
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-5 md:space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onBack}
            className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 md:px-6 py-1.5 sm:py-2 md:py-2.5 rounded-lg text-sm font-medium transition ${isDark
              ? "bg-slate-800 text-white hover:bg-slate-700"
              : "bg-slate-200 text-slate-900 hover:bg-slate-300"
              }`}
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3 md:gap-4">
          ...
          <div className={`p-3 sm:p-4 md:p-6 rounded-xl border transition-all hover:scale-[1.02] ${totalPnL >= 0
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
              ...
              <span className={`text-[10px] sm:text-xs md:text-sm font-medium ${totalPnL >= 0
            ...
            <div className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold ${totalPnL >= 0
              ? isDark ? "text-green-400" : "text-green-700"
              : isDark ? "text-red-400" : "text-red-700"
              }`}>
        ...
        <div className={`rounded-xl border p-3 sm:p-4 md:p-6 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"
          ...
          <div className="relative w-full" style={{ height: 'clamp(250px, 50vw, 350px)' }}>
            ...
            <div className="absolute left-0 top-0 bottom-8 sm:bottom-10 md:bottom-12 w-12 sm:w-16 md:w-20 flex flex-col justify-between text-[10px] sm:text-xs">
              <div className={`text-right pr-1.5 sm:pr-2 md:pr-3 ${isDark ? "text-slate-400" : "text-slate-600"}`}>
            ...
            {/* Chart area */}
            <div className="ml-12 sm:ml-16 md:ml-20 mr-2 sm:mr-4 h-full pb-8 sm:pb-10 md:pb-12 relative">
              {/* Tooltip - Mobile Optimized */}
              {hoveredPoint && (
                <div
                  className={`absolute z-50 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg shadow-xl border pointer-events-none whitespace-nowrap ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-slate-200"
                    }`}
                  style={{
                ...
                >
                  ...
                  <div className={`text-base sm:text-lg font-bold ${hoveredPoint.pnl >= 0
              ...
              >
                ...
                {/* Interactive data points - Touch Optimized */}
                {equityCurve.map((d, i) => {
                  const x = (i / Math.max(1, equityCurve.length - 1)) * 1000;
                  const y = ((maxPnL - d.cumulativePnL) / range) * 300;
                  return (
              ...
              {/* X-axis labels - Mobile Optimized with Rotation */}
              <div className="absolute bottom-0 left-0 right-0 flex justify-between px-1">
                {equityCurve
                  ...
                  .map((d, idx) => (
                    <div
                      key={`label-${idx}`}
                      className={`text-[9px] sm:text-[10px] md:text-xs ${isDark ? "text-slate-400" : "text-slate-600"
                        }`}
                      style={{
        ...
        {/* Daily P&L Bar Chart - Mobile Optimized */}
        <div className={`rounded-xl border p-3 sm:p-4 md:p-6 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"
          }`}>
          <h2 className={`text-base sm:text-lg md:text-xl font-bold mb-3 sm:mb-4 md:mb-6 ${isDark ? "text-white" : "text-slate-900"
            }`}>
          ...
          <div className="relative w-full" style={{ height: 'clamp(220px, 45vw, 320px)' }}>
            {(() => {
              ...
              const maxScale = Math.max(maxPositive, maxNegative, 100);
                ...
                <>
                  ...
                  <div className="absolute left-0 top-0 bottom-8 sm:bottom-10 md:bottom-12 w-12 sm:w-16 md:w-20 flex flex-col justify-between text-[10px] sm:text-xs">
                    <div className={`text-right pr-1.5 sm:pr-2 md:pr-3 ${isDark ? "text-slate-400" : "text-slate-600"
                      ...
                      ₹{(maxScale / 1000).toFixed(0)}k
                    </div>
                    <div className={`text-right pr-1.5 sm:pr-2 md:pr-3 font-bold ${isDark ? "text-slate-300" : "text-slate-700"
                      }`}>
                      ₹0
                    </div>
                    <div className={`text-right pr-1.5 sm:pr-2 md:pr-3 ${isDark ? "text-slate-400" : "text-slate-600"
                  ...
                  {/* Chart area */}
                  <div className="ml-12 sm:ml-16 md:ml-20 mr-2 sm:mr-4 h-full relative pb-8 sm:pb-10 md:pb-12">
                    {/* Grid lines */}
                    <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
                      <div className={`w-full h-px ${isDark ? "bg-slate-700" : "bg-slate-200"}`} />
                      <div className={`w-full h-px ${isDark ? "bg-slate-700" : "bg-slate-200"}`} />
                      <div className={`w-full h-0.5 ${isDark ? "bg-slate-600" : "bg-slate-400"}`} />
                      <div className={`w-full h-px ${isDark ? "bg-slate-700" : "bg-slate-200"}`} />
                      <div className={`w-full h-px ${isDark ? "bg-slate-700" : "bg-slate-200"}`} />
                    </div>

                    {/* Bars - Mobile Optimized */}
                    <div className="absolute inset-0 flex items-center justify-between gap-0.5 sm:gap-1 md:gap-1.5">
                      {Object.entries(dailyPnL)
                        .sort(([a], [b]) => new Date(a).getTime() - new Date(b).getTime())
                        .map(([date, pnl]) => {
                          const isPositive = pnl >= 0;
                          const heightPercentOfHalf = (Math.abs(pnl) / maxScale) * 100;

                          return (
                            <div
                              key={date}
                              className="flex-1 h-full flex flex-col justify-center items-stretch group relative"
                              style={{ minWidth: '3px', maxWidth: '50px' }}
                            >
                              {/* Tooltip - Mobile Optimized */}
                              <div
                                className={`absolute ${isPositive ? 'bottom-[55%] mb-2 sm:mb-3' : 'top-[55%] mt-2 sm:mt-3'
                              ...
                              <div className="flex-1 flex flex-col">
                                <div className="flex-1 flex flex-col justify-end items-stretch">
                                  {isPositive && (
                                    <div
                                      className="w-full rounded-t-md transition-all duration-300 bg-green-500 hover:bg-green-400 active:bg-green-600 cursor-pointer"
                                      style={{
                                        height: `${heightPercentOfHalf}%`,
                                ...
                                <div className="flex-1 flex flex-col justify-start items-stretch">
                                  {!isPositive && (
                                    <div
                                      className="w-full rounded-b-md transition-all duration-300 bg-red-500 hover:bg-red-400 active:bg-red-600 cursor-pointer"
                                      style={{
                                        height: `${heightPercentOfHalf}%`,
                                        minHeight: pnl < 0 ? '2px' : '0'
                                      }}
        ...
        {/* Performance Metrics - Mobile Stacked */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
          ...
          <div className={`rounded-xl border p-3 sm:p-4 md:p-6 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between py-2 border-b last:border-0" style={{
