// src/app/dashboard/trading/strategies/components/EditStrategyModal.tsx
"use client";

import { useState, useEffect } from "react";
import { X } from "lucide-react";
import type { Strategy, StrategyStatus, MarketCondition } from "@/types/database";

interface EditStrategyModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (strategyId: string, updates: Partial<Strategy>) => Promise<{ success: boolean }>;
...
export default function EditStrategyModal({
  ...
  useEffect(() => {
    if (strategy) {
      ...
      setMarketType(strategy.market_type || "trending");
      setTimeframe(strategy.timeframe || "15m");
      setStatus(strategy.status);
      setEntryCriteria(strategy.entry_criteria?.description || "");
      setExitCriteria(strategy.exit_criteria?.description || "");
      setRiskManagement(strategy.risk_management || "");
    }
  }, [strategy]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await onUpdate(strategy.id, updates);
