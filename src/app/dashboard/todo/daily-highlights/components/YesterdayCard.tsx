    ...
    <div className={`rounded-xl border p-4 sm:p-5 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
      ...
      {snapshot ? (
        <div className="space-y-3">
          {/* Tasks Completed */}
          <div className={`flex items-center justify-between p-3 rounded-lg ${isDark ? 'bg-slate-700/50' : 'bg-slate-50'
            }`}>
            <div className="flex items-center gap-2">
              <CheckCircle2 className={`w-4 h-4 sm:w-5 sm:h-5 ${isDark ? 'text-green-400' : 'text-green-600'
                }`} />
          ...
          <div className={`flex items-center justify-between p-3 rounded-lg ${isDark ? 'bg-slate-700/50' : 'bg-slate-50'
            ...
            <div className="flex items-center gap-2">
              <Clock className={`w-4 h-4 sm:w-5 sm:h-5 ${isDark ? 'text-indigo-400' : 'text-indigo-600'
                }`} />
              <span className={`text-sm font-medium ${isDark ? 'text-slate-300' : 'text-slate-700'
                }`}>
                Time Focused
