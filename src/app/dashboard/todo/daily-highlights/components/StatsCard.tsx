...
export default function StatsCard({ streak, highlightCompleted, isDark }: StatsCardProps) {
  ...
  return (
    <div className={`rounded-xl border p-6 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
      ...
      <h3 className={`text-lg font-bold flex items-center gap-2 mb-6 ${isDark ? 'text-white' : 'text-slate-900'
        }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className={`p-5 rounded-xl text-center relative overflow-hidden ${streak > 0
          ...
          <div className="relative z-10">
            <Flame className={`w-8 h-8 mx-auto mb-2 ${streak > 0 ? 'text-orange-200 animate-pulse' : isDark ? 'text-slate-600' : 'text-slate-300'
              }`} />
            <div className={`text-3xl font-bold mb-1 ${streak > 0 ? 'text-orange-200' : isDark ? 'text-white' : 'text-slate-900'
              }`}>
              {streak}
            </div>
            <div className={`text-xs font-medium ${isDark ? 'text-orange-200' : 'text-slate-600'
              }`}>
              Day Streak
            </div>
            {streak > 0 && (
        ...
        {/* Today's Status */}
        <div className={`p-5 rounded-xl text-center ${highlightCompleted
            ? isDark
          ...
          <Target className={`w-8 h-8 mx-auto mb-2 ${highlightCompleted
              ? 'text-green-500'
              : isDark
                ? 'text-slate-600'
                : 'text-slate-300'
            }`} />
          <div className={`text-3xl font-bold mb-1 ${highlightCompleted
              ? 'text-green-500'
              : isDark
                ? 'text-white'
                : 'text-slate-900'
        ...
        {/* Level Badge */}
        <div className={`p-5 rounded-xl text-center ${isDark ? 'bg-gradient-to-br from-purple-900/50 to-pink-900/50' : 'bg-gradient-to-br from-purple-50 to-pink-50'
          }`}>
          <Award className={`w-8 h-8 mx-auto mb-2 ${isDark ? 'text-purple-400' : 'text-purple-600'
            }`} />
          <div className={`text-2xl font-bold mb-1 ${isDark ? 'text-purple-300' : 'text-purple-700'
            }`}>
            {streak >= 30 ? 'Pro' : streak >= 14 ? 'Expert' : streak >= 7 ? 'Rising' : 'Starter'}
          </div>
          <div className={`text-xs font-medium ${isDark ? 'text-purple-400' : 'text-purple-600'
            }`}>
            Level
          </div>
        </div>
        ...
        {/* Motivation */}
        <div className={`p-5 rounded-xl text-center ${isDark ? 'bg-gradient-to-br from-indigo-900/50 to-blue-900/50' : 'bg-gradient-to-br from-indigo-50 to-blue-50'
          }`}>
          <div className="text-3xl mb-2">
            {highlightCompleted ? "🎉" : streak > 0 ? "💪" : "🚀"}
          </div>
          <div className={`text-sm font-bold ${isDark ? 'text-indigo-300' : 'text-indigo-700'
            }`}>
            {highlightCompleted
              ? "Amazing!"
              : streak >= 7
      ...
      {/* Streak Message */}
      {streak > 0 && (
        <div className={`mt-4 p-4 rounded-lg text-center relative overflow-hidden ${isDark
          ...
          <p className={`text-sm font-bold relative z-10 ${isDark ? 'text-orange-300' : 'text-orange-700'
            }`}>
            {streak >= 30
      ...
      {streak === 0 && !highlightCompleted && (
        <div className={`mt-4 p-4 rounded-lg text-center ${isDark
