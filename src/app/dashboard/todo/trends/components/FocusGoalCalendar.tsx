  ...
  return (
    <div className={`rounded-xl border p-3 sm:p-4 md:p-6 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"}`}>
      ...
      <div className="mb-3 sm:mb-4 md:mb-6">
        ...
        {/* Stats - Mobile: Wrap */}
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] sm:text-xs md:text-sm">
          <div>
            <span className={isDark ? "text-slate-400" : "text-slate-600"}>Focus Days:</span>
            <span className={`ml-1 font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{totalGoalDays}</span>
          </div>
          <div>
            <span className={isDark ? "text-slate-400" : "text-slate-600"}>Completed:</span>
            <span className={`ml-1 font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{completedDays}</span>
          </div>
          <div>
            <span className={isDark ? "text-slate-400" : "text-slate-600"}>Rate:</span>
            <span className={`ml-1 font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{completionRate.toFixed(0)}%</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <button onClick={handlePrevMonth} className={`p-1.5 sm:p-2 rounded hover:bg-slate-700 transition ${isDark ? "text-slate-300" : "text-slate-600"}`}>
            <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <h3 className={`text-sm sm:text-base md:text-lg font-bold ${isDark ? "text-white" : "text-slate-900"}`}>
            {monthNames[selectedMonth]} {selectedYear}
          </h3>
          <button onClick={handleNextMonth} className={`p-1.5 sm:p-2 rounded hover:bg-slate-700 transition ${isDark ? "text-slate-300" : "text-slate-600"}`}>
        ...
        <div className="grid grid-cols-7 gap-0.5 sm:gap-1 mb-1.5 sm:mb-2">
          {dayNames.map((day) => (
            <div key={day} className={`text-center text-[9px] sm:text-[10px] md:text-xs font-medium py-1 ${isDark ? "text-slate-400" : "text-slate-600"}`}>
              {day}
            </div>
          ))}
        </div>

        {/* Calendar Grid - Mobile: Smaller cells */}
        <div className="grid grid-cols-7 gap-0.5 sm:gap-1">
          {days.map((day, index) => {
            ...
            const percentage = hasGoal ? Math.min((goalData.hoursWorked / goalData.goalHours) * 100, 100) : 0;
        ...
        <div className="mt-3 sm:mt-4 flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 text-[9px] sm:text-[10px] md:text-xs">
          ...
          <div className="flex items-center gap-1">
            <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 rounded-full border-2 border-orange-500" />
            <span className={isDark ? "text-slate-400" : "text-slate-600"}>In Progress</span>
          </div>
          <div className="flex items-center gap-1">
            <div className={`w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 rounded-full border-2 ${isDark ? "border-slate-600" : "border-slate-300"}`} />
            <span className={isDark ? "text-slate-400" : "text-slate-600"}>No Goal</span>
          </div>
        </div>

        {/* Selected Day Info - Mobile: Compact */}
        {selectedDayData && (
          <div className={`mt-3 sm:mt-4 p-3 sm:p-4 rounded-lg border ${isDark ? "bg-slate-700 border-slate-600" : "bg-slate-50 border-slate-200"}`}>
            ...
            <div className="space-y-1 text-[10px] sm:text-xs md:text-sm">
              <div className="flex justify-between">
                <span className={isDark ? "text-slate-400" : "text-slate-600"}>Hours Worked:</span>
                <span className={`font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{selectedDayData.hoursWorked.toFixed(1)}h</span>
              </div>
              <div className="flex justify-between">
                <span className={isDark ? "text-slate-400" : "text-slate-600"}>Goal:</span>
                <span className={`font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{selectedDayData.goalHours}h</span>
              </div>
              <div className="flex justify-between">
                <span className={isDark ? "text-slate-400" : "text-slate-600"}>Progress:</span>
                <span className={`font-bold ${selectedDayData.goalMet ? "text-green-500" : "text-orange-500"}`}>
                  {Math.min((selectedDayData.hoursWorked / selectedDayData.goalHours) * 100, 100).toFixed(0)}%
                </span>
              </div>
            </div>
          </div>
