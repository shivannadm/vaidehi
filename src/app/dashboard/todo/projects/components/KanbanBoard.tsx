...
export default function KanbanBoard({ tasks, onRefresh, isDark }: KanbanBoardProps) {
  ...
  return (
    <div className="p-3 md:p-6">
        ...
        {columns.map((column) => {
          ...
          return (
            <div
              ...
              className={`rounded-xl border-2 p-3 md:p-4 min-h-[200px] md:min-h-96 transition-all ${isOver
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3 md:mb-4">
                <div className="flex items-center gap-1.5 md:gap-2">
                  ...
                  <span
                    className={`px-1.5 md:px-2 py-0.5 rounded-full text-[10px] md:text-xs font-bold ${isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-200 text-slate-700'
                      }`}
                  >
                    {columnTasks.length}
                  </span>
              ...
              <div className="space-y-2 md:space-y-3">
                {columnTasks.length === 0 ? (
                  <div className={`text-center py-6 md:py-8 text-xs md:text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'
                    }`}>
                  ...
                  columnTasks.map((task) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group rounded-lg p-2.5 md:p-3 border cursor-move transition-all hover:shadow-md ${isDragging
                            ? 'opacity-50 scale-95'
                            : task.is_completed
                              ? isDark
                      ...
                      >
                        ...
                        <div className="flex items-start justify-between gap-2 mb-1.5 md:mb-2">
                          <div className="flex items-start gap-1.5 md:gap-2 flex-1 min-w-0">
                            <GripVertical className={`w-3 h-3 md:w-4 md:h-4 mt-0.5 flex-shrink-0 ${isDark ? 'text-slate-500' : 'text-slate-400'
                              }`} />
                            <div className="flex-1 min-w-0">
                              <h4
                                className={`font-semibold text-xs md:text-sm mb-1 break-words ${task.is_completed
                                    ? isDark
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            ...
                            <button
                              ...
                              className={`p-1 rounded md:opacity-0 md:group-hover:opacity-100 transition ${isDark ? 'hover:bg-slate-600' : 'hover:bg-slate-100'
                                }`}
                            >
      ...
      <div className={`mt-4 md:mt-6 p-3 md:p-4 rounded-lg border text-xs md:text-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-blue-50 border-blue-200'
        }`}>
