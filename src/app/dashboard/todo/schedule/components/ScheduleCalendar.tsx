...
export default function ScheduleCalendar({
  ...
  return (
    <div
      className={`rounded-xl border p-3 sm:p-4 md:p-5 h-full flex flex-col ${isDark
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1">
          <button
            onClick={onPrevMonth}
            className={`p-1 sm:p-1.5 rounded-lg transition ${isDark
      ...
      <div className="grid grid-cols-7 gap-0.5 sm:gap-1 mb-1 sm:mb-2">
        {weekDays.map((day) => (
          <div
            key={day}
            className={`text-center text-[10px] sm:text-xs font-semibold py-1 sm:py-2 ${isDark ? "text-slate-400" : "text-slate-500"
              }`}
          >
            {day}
          </div>
                  ...
                  {eventCount > 0 && (
                    <div className="flex gap-0.5 mt-0.5 sm:mt-1">
                      {[...Array(Math.min(eventCount, 3))].map((_, i) => (
                        <div
                          key={i}
                          className={`w-0.5 h-0.5 sm:w-1 sm:h-1 rounded-full ${isSelected
                              ? "bg-white"
                              : isDark
                                ? "bg-indigo-400"
                                : "bg-indigo-500"
      ...
      <button
        ...
        className={`w-full px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg font-semibold text-xs sm:text-sm transition ${isDark
