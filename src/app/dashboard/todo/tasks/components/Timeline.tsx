...
export default function Timeline({ sessions = [], currentTime, isDark }: TimelineProps) {
  // Smart timeline range - show reasonable window
    ...
    sessions.forEach(session => {
      ...
      if (session?.end_time) {
        try {
          ...
          console.warn("Invalid end_time:", session.end_time);
        }
      }
    });
    
    // Default range: 2 hours before current to current hour
    const startHour = Math.max(0, Math.min(earliestHour, currentHour - 2));
    const endHour = Math.min(23, Math.max(latestHour, currentHour));
    
    const hours = [];
    for (let i = startHour; i <= endHour + 1; i++) { // +1 to show next hour
  ...
  const getSessionPosition = (session: TaskSessionWithTask) => {
    try {
      ...
      const heightPercent = (durationMinutes / timelineTotalMinutes) * 100;
  ...
  return (
    <div className="relative h-full">
      ...
      <div className={`relative h-full overflow-y-auto ${isDark ? 'scrollbar-custom' : 'scrollbar-custom-light'}`}>
        <div className="relative pr-2" style={{ minHeight: `${totalHeight}px` }}>
          ...
          <div className="ml-14 relative" style={{ height: `${totalHeight}px` }}>
            ...
            {/* Current Time Line - STOPS AT CURRENT TIME */}
            {currentTimePos && (
              <div
