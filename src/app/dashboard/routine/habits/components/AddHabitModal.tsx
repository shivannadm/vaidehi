// CREATE: src/app/dashboard/routine/habits/components/AddHabitModal.tsx
"use client";
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              disabled={saving}
              className={`w-full px-4 py-3 rounded-lg border text-base resize-none transition ${
                isDark
                  ? "bg-slate-700 border-slate-600 text-white placeholder-slate-400 focus:border-indigo-500"
                  : "bg-white border-slate-300 text-slate-900 placeholder-slate-400 focus:border-indigo-500"
              } focus:outline-none focus:ring-2 focus:ring-indigo-500/30`}
            ...
            <div className="grid grid-cols-10 gap-2">
              {POPULAR_ICONS.map((emoji) => (
                <button
                  ...
                  disabled={saving}
                  className={`aspect-square text-2xl rounded-lg border-2 transition hover:scale-110 ${
                    icon === emoji
              ...
              {COLORS.map((c) => (
                <button
                  ...
                  className={`aspect-square rounded-lg border-2 transition hover:scale-110 ${
                    color === c.value
                      ? "border-white ring-2 ring-offset-2 ring-indigo-600"
                      : "border-transparent"
                  }`}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className={`text-sm ${isDark ? "text-slate-400" : "text-slate-600"}`}>
                  {category} • {targetCount}x per {frequency === "daily" ? "week" : frequency}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="flex-1 px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-lg font-medium transition disabled:opacity-50"
            >
