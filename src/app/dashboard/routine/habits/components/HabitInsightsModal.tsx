...
const LIFE_CHANGING_HABITS: Array<{
  id: number;
  habit: string;
  description: string;
  category: string;
...
export default function HabitInsightsModal({ onClose, onAddHabit, isDark }: HabitInsightsModalProps) {
  ...
  const handleAddHabit = async (habitData: any) => {
    ...
    try {
      await onAddHabit({
        ...
        category: (habitData.category || "Custom") as HabitCategory,
      ...
      // small UX delay to show added state
      setTimeout(() => setAdding(null), 450);
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className={`rounded-2xl w-full max-w-4xl my-8 shadow-2xl border ${isDark ? "bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-slate-700" : "bg-gradient-to-br from-white via-slate-50 to-white border-slate-200"}`}>
        {/* Header */}
        <div className={`sticky top-0 z-10 rounded-t-2xl ${isDark ? "bg-slate-900/95" : "bg-white/95"}`}>
          <div className="flex items-center justify-between p-3 md:p-4 border-b border-slate-700/50">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="absolute inset-0 blur-lg opacity-40" style={{ background: "linear-gradient(to right,#ef4444,#f97316)" }} />
                <div className="relative p-2 rounded-lg bg-gradient-to-br from-red-600 to-orange-600">
                  <Sparkles className="w-5 h-5 text-white" />
                </div>
              </div>
              <div>
                <h2 className={`text-lg md:text-xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>HABITS THAT WILL</h2>
                <h3 className="text-lg md:text-xl font-black bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">CHANGE YOUR LIFE</h3>
              </div>
            </div>
            <button onClick={onClose} className={`p-2 rounded-lg transition-all hover:scale-110 ${isDark ? "bg-slate-800 hover:bg-slate-700" : "bg-slate-100 hover:bg-slate-200"}`}>
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="px-3 py-2">
            <p className={`text-xs ${isDark ? "text-slate-400" : "text-slate-600"}`}>
              <span className="font-semibold text-orange-500">Sample list</span> — click "Add Habit" to track any of these instantly. Replace the sample array with your full set if you have it.
            </p>
          </div>
        </div>

        {/* Content */}
        <div className="p-3 md:p-4 space-y-3 max-h-[calc(100vh-220px)] overflow-y-auto">
          {LIFE_CHANGING_HABITS.map((item, index) => (
            <div key={item.id} className={`group relative rounded-lg p-3 transition-all hover:scale-[1.01] cursor-pointer border ${isDark ? "bg-slate-800/60 border-slate-700/50 hover:border-orange-500/30 hover:bg-slate-800" : "bg-slate-50 border-slate-200 hover:border-orange-500/30 hover:bg-white"}`}>
              <div
                className={`absolute -top-1 -left-1 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${isDark ? "bg-slate-700 text-slate-400" : "bg-slate-200 text-slate-600"}`}>
                {index + 1}
              </div>

              <div className="absolute top-2 right-2">
                <span className="text-[9px] px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400 font-semibold border border-orange-500/20">{item.category}</span>
              </div>

              <div className="flex items-center gap-3">
                <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-xl ${isDark ? "bg-slate-700/50" : "bg-white shadow-sm"}`}>{item.icon}</div>

                <div className="flex-1 min-w-0 pr-24">
                  <div className="flex items-baseline gap-2 mb-1">
                    <h4 className={`text-sm font-bold ${isDark ? "text-white" : "text-slate-900"}`}>{item.habit}</h4>
                    {item.benefit && <span className="text-xs font-bold text-orange-500">{item.benefit}</span>}
                  </div>
                  <p className={`text-xs leading-relaxed line-clamp-2 ${isDark ? "text-slate-400" : "text-slate-600"}`}>{item.description}</p>
                </div>

                <button
                  onClick={() => handleAddHabit(item)}
                  disabled={adding === item.id}
                  className={`absolute right-3 top-1/2 -translate-y-1/2 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${adding === item.id ? "bg-green-500 text-white" : "bg-gradient-to-r from-orange-500 to-red-500 text-white hover:shadow-xl hover:scale-105 active:scale-95"}`}
                >
        ...
        <div className={`p-3 md:p-4 border-t rounded-b-2xl ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-slate-50 border-slate-200"}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="flex -space-x-1">
                {["🔥", "💪", "🧠"].map((emoji, i) => (
                  <div key={i} className={`w-7 h-7 rounded-full flex items-center justify-center text-sm border ${isDark ? "bg-slate-700 border-slate-600" : "bg-white border-slate-200"}`}>{emoji}</div>
                ))}
              </div>
              <p className={`text-xs ${isDark ? "text-slate-300" : "text-slate-600"}`}>Start with 3-5 habits and build from there</p>
            </div>
            <button onClick={onClose} className="px-3 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white text-sm font-semibold transition-all hover:scale-105">Done</button>
          </div>
        </div>
