...
export async function captureTradingScreenshot(
  elementId: string = 'trading-analytics-dashboard',
  options?: TradingExportOptions
): Promise<void> {
  console.log('📸 Capturing trading analytics screenshot...');
  ...
  try {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        link.download = `trading-analytics-${timestamp}.png`;
        link.href = url;
...
export async function generateTradingPDF(
  ...
  try {
    ...
    doc.text('Trading Analytics Report', leftMargin, 35);
    ...
    const timestamp = new Date().toISOString().split('T')[0];
    doc.save(`trading-report-${timestamp}.pdf`);
